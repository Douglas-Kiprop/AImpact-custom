...
function ChatInterface() {
  ...
  // const [isLoading, setIsLoading] = useState(false); // Removed: New state for loading indicator
  ...
  ], []); // Empty dependency array means it will only be memoized once
  ...
  const handleSendMessage = async () => {
    if (input.trim()) {
      ...
      setInput('');
      // setIsLoading(true); // Removed: Set loading to true when message is sent
      
      // Add a temporary 'thinking' message immediately
      const thinkingMessage = { text: "Thinking...", sender: 'ai', timestamp: new Date(), isTemporary: true };
      ...
      try {
        ...
        const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/message`, { // Corrected API endpoint
          method: 'POST',
        ...
        // Remove the temporary 'thinking' message and add the actual AI response
        ...
        setMessages(prev => prev.filter(msg => !msg.isTemporary));
        setMessages(prev => [...prev, { text: "Sorry, something went wrong. Please try again.", sender: 'ai', timestamp: new Date() }]);
      } // Removed: finally block is no longer needed as isLoading is removed
    }
  };
              ...
              {messages.map((msg, index) => (
                ...
                >
                  <div className={`flex items-start space-x-3 max-w-2xl ${msg.sender === 'user' ? 'flex-row-reverse space-x-reverse' : ''}`}>
                    ...
                    } ${msg.isTemporary ? 'animate-pulse' : ''}`}> {/* Add animate-pulse for temporary messages */}
                      <p className="text-sm leading-relaxed">{msg.text}</p>
                    </div>
                  </div>
                </div>
              ))}
              {/* Removed the isLoading conditional rendering block that was causing the error */}
            </div>
          </div>
